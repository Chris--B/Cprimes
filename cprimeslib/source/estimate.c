...
/* pi(num) == pi_lookup[num] if num is small enough to be in the lookup table */
...
CPRIMES_DEC size_t low_estimate(uint64_t num) {
	...
	assert(num >= 599);
	return num / log(num) * (1 + 1 / log(num));
}
...
CPRIMES_DEC size_t high_estimate(uint64_t num) {
	...
	if (55 <= num && num < 355991) {
		return num / (log(num) - 4);
	}
	return num / log(num) * (1 + 1 / log(num) + 2.51 / (log(num) * log(num)));
}
