...
#define define_estimate_test(num)                                         \
	void estimate_##num(CuTest* tc) {                                     \
		...
		good_res = good_estimate(num);                                    \
		                                                                  \
		CuAssertf(tc, low_res <= good_res,                                \
			"good is less than low! (%"PRIu64 " < " "%" PRIu64 ")",       \
			good_res, low_res);                                           \
		CuAssertf(tc, low_res <= high_res,                                \
			"low is greater than high! (%"PRIu64 " > " "%" PRIu64 ")",    \
			low_res, high_res);                                           \
		CuAssertf(tc, good_res <= high_res,                               \
			"good is greater than high! (%"PRIu64 " > " "%" PRIu64 ")",   \
			good_res, high_res);                                          \
	}                                                                     \
...
#define define_eratos_in_estimate_bounds_test(num)                        \
	void eratos_in_estimate_bounds_##num(CuTest* tc) {                    \
		...
		CuAssertf(tc, err == 0,                                           \
			"eratos(%" PRIu64") failed: %s",                              \
			(uint64_t)num, strerror(err));                                \
		free(primes);                                                     \
		                                                                  \
		low_res = low_estimate(num);                                      \
		high_res = high_estimate(num);                                    \
		                                                                  \
		CuAssertf(tc, low_res <= actual,                                  \
			"low_estimate is too large, "                                 \
			"or eratos is broken (%"PRIu64" < %"PRIu64")",                \
			(uint64_t)actual, low_res );                                  \
		CuAssertf(tc, actual <= high_res,                                 \
			"high_estimate is too small, "                                \
			"or eratos is broken (%"PRIu64" > %"PRIu64")",                \
			(uint64_t)actual, high_res );                                 \
	}                                                                     \
